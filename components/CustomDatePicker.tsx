...
export const CustomDatePicker: React.FC<Props> = ({
  ...
  const handleInputClick = () => {
    if (inputRef.current) {
      ...
      const dropdownHeight = 380; // 预估下拉框高度
  ...
  return (
    <div ref={dropdownRef} className={`relative w-full ${className}`}>
      ...
      {isOpen && (
        <div
          className={`absolute z-50 w-full bg-white border-2 border-black rounded-md shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] ${
          ...
          <div className="p-3">
            ...
            <div className="grid grid-cols-7 gap-1">
              {calendarDays.map((date, index) => {
                ...
                const { day, isToday, isSelected } = formatDateForDisplay(date);
